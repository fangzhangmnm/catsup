...
describe("camera persp", () => {
  ...
  it("target 投影在屏幕中心；眼距 = halfH / tan(fov/2)", () => {
    const c = persp();
    const s = c.angularPx(c.target, VP);
  ...
  it("近大远小：同样的世界长度，离眼越近屏幕越长", () => {
    ...
    const la = Math.hypot(c.angularPx(a1, VP).x - c.angularPx(a0, VP).x, c.angularPx(a1, VP).y - c.angularPx(a0, VP).y);
    const lb = Math.hypot(c.angularPx(b1, VP).x - c.angularPx(b0, VP).x, c.angularPx(b1, VP).y - c.angularPx(b0, VP).y);
    assert(lb > la * 1.2, `near ${lb} vs far ${la}`);
  });

  it("viewDirAt：正交=常向量；透视=指向眼睛的单位向量", () => {
    const o = new OrbitCamera();
    const d1 = o.viewDir(P(0, 0)), d2 = o.viewDir(P(100, 100, 50));
