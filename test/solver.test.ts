...
describe("solver: property 不变量", () => {
  it("解点永远落在光标合成半径邻域内（全屏扫描）", () => {
    ...
    for (let i = 0; i < 200; i++) {
      ...
      const sol = solvePoint({ cam: c, vp: VP }, { x: sx, y: sy }, C);
      assert(sol !== null, "兜底平面保证有解");
      if (sol!.dim === 2) continue; // 兜底=光标射线本身
      const sp = c.worldToScreen(sol!.p, VP);
  ...
  it("确定性：同输入两次求解逐字节同解", () => {
    ...
    for (const [sx, sy] of [[500, 400], [520, 380], [100, 700]]) {
      const a = solvePoint({ cam: c, vp: VP }, { x: sx, y: sy }, C);
      const b = solvePoint({ cam: c, vp: VP }, { x: sx, y: sy }, C);
  ...
  it("解释完整性：used 非空且 kind/hints 全部派生自 used", () => {
    ...
    const s = c.worldToScreen({ x: 0.3, y: 20.2, z: 0 }, VP); // 靠近顶边端点/共轴区
    const sol = solvePoint({ cam: c, vp: VP }, s, C)!;
  ...
  it("字典序：维度优先（合成 0-D 压过单 1-D）、同维 rank 优先（端点压过更近的中点）", () => {
    ...
    const near = c.worldToScreen({ x: 19.4, y: 0.2, z: 0 }, VP);
    const C = buildConstraints({ k, plane: GROUND.plane });
    const sol = solvePoint({ cam: c, vp: VP }, near, C)!;
    ...
    const s2 = c.worldToScreen({ x: 20.2, y: 8.3, z: 0 }, VP); // 右边界边上、anchor 的 X 轴线附近
    const sol2 = solvePoint({ cam: c, vp: VP }, s2, C2)!;
