...
//
// 键位（user：「相机 wasd 不够，还需要 qe 上下」+ handoff）：WASD 走 · Q/E 下/上（noclip 飞） · Shift 冲刺 · 空格 跳 · Ctrl 蹲 ·
...
import { type HeadFrame, type InputFrame, emptyInput } from "./input.ts";
...
export class FlatInput {
  ...
  read(head: HeadFrame, aim: Ray | null): InputFrame {
    const d = this.down;
    const f = emptyInput();
