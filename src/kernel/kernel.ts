...
import { type Pt, dist, distToSegment, pointOnSegment, quantize, samePt, segIntersections } from "./geom.ts";
...
export class Kernel {
  ...
  /** 构造手势：一批线段（Rect = 4 段）。 */
  addEdges(segs: readonly (readonly [Pt, Pt])[]): FaceEvent[] {
    const gesture = new Set<EdgeId>();
    for (const [a, b] of segs) {
      const r = insertSegment(this.graph, a, b, (parent, c1, c2) => {
        // 手势边被后续段切开 → 子边继承手势身份
    ...
    return this.emit(this.store.reconcileConstructive(this.graph, gesture));
  }
  ...
  eraseEdges(ids: readonly EdgeId[]): FaceEvent[] {
    ...
    return this.emit(this.store.reconcileErase(this.graph, verdicts));
  }
   ...
   */
  moveVertices(moves: readonly { id: VertexId; to: Pt }[]): FaceEvent[] {
    ...
    for (const mv of moves) {
      ...
      const target = quantize(mv.to);
      const cur = this.graph.pt(vid);
      if (samePt(cur, target)) continue;
    ...
    return this.emit(this.store.reconcileMove(this.graph, ringVerts, mergeMap, topologyChanged, movedFaces));
  }
  ...
  /** target 落在哪条边内部（排除 vid 的 incident 边——顶点滑到自家边上是折叠，M1 不处理）。 */
  private findEdgeContaining(target: Pt, vid: VertexId): EdgeId | undefined {
    for (const e of this.graph.edges()) {
      if (e.a === vid || e.b === vid) continue;
      const pa = this.graph.pt(e.a), pb = this.graph.pt(e.b);
      if (!pointOnSegment(target, pa, pb)) continue;
      if (samePt(target, pa) || samePt(target, pb)) continue;
      return e.id;
    }
    return undefined;
  }
  ...
  private dirtyIncidentEdges(vid: VertexId): EdgeId[] {
    ...
    for (const eid of this.graph.vertex(vid).edges) {
      ...
      for (const o of this.graph.edges()) {
        ...
        if (segIntersections(pa, pb, this.graph.pt(o.a), this.graph.pt(o.b)).length) { isDirty = true; break; }
      }
      if (!isDirty) {
        for (const v of this.graph.vertices()) {
          if (v.id === e.a || v.id === e.b) continue;
          if (pointOnSegment({ x: v.x, y: v.y }, pa, pb)) { isDirty = true; break; }
        }
      }
