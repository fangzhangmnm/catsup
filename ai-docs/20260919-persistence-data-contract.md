...
其余要点：
- **本体（user 09-20）：「小朋友糊房子和 N64 里面面的语义就是墙。而不是靠闭流形做实体墙」——膜 = 两面的实体（墙），闭流形只是特例；正反永远是渲染 / 导出层派生，不是几何状态。**
