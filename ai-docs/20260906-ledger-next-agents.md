...
### A18 转正纪元（0.5.0）：接 store + gallery，开 OneDrive appfolder 仓 — `开工 2026-09-20（user「嗯应该可以开写了」；Claude Fable 5.1）`：步骤 ① `src/format/`（本 session）→ ② store/gallery → ③ 头片 peek escalate → ④ A1 undo/workpiece → ⑤ QoL（user 2026-09-19：「最重要的，接 store 和 gallery，开 onedrive 仓。这个才是我能长期做的」）
