...
### A18 转正纪元（0.5.0）：接 store + gallery，开 OneDrive appfolder 仓 — `开工 2026-09-20（user「嗯应该可以开写了」；Claude Fable 5.1）`：步骤 ① `src/format/` **done v0.5.0（2026-09-20）** → ② store/gallery → ③ 头片 peek escalate → ④ A1 undo/workpiece → ⑤ QoL
- **① 落地记录（v0.5.0，Claude Fable 5.1）**：`src/format/{binary,glb,brep-codec,triangulate,bake,document,read,write,index}.ts` + `migrate/{index,gltf/index}.ts`（纯模块，零 DOM / 零 three；vendored `src/vendor/earcut` 3.0.1 ISC 只给 bake 三角化）；内核新入口 `Kernel.fromBrep / toBrep`（`BrepSnapshot`；`PlaneRegistry.restore`、`FaceStore.restoreFace`；立宪页 §5「持久化装载」已回写）；测试 `test/{format-binary,kernel-brep,format}.test.ts` + golden 场景 `test/format-scenes.ts`（box / holed / house）+ **冻结样本 `test/fixtures/format/v1/*.glb`**（`scripts/freeze-format-fixtures.mjs`，老样本永不改写）+ 体积预算断言；259 绿。**v1 范围**：单 root definition、无材质 / 属性层（只保留未知）、bake = float32 POSITION 无 NORMAL + TRIANGLES + LINES、缩略图槽已留（app 侧渲染归 ②）。**已知待办**：未知扩展的二进制 bufferView 在重写时不带（round-trip 保真 v1 限 JSON，契约 §7 第 1 条的缺口，接 store 前补）；`asset.version 2.1` 属性名以 draft-2.1 为准，定稿若改名走 migrate/gltf。（user 2026-09-19：「最重要的，接 store 和 gallery，开 onedrive 仓。这个才是我能长期做的」）
...
- **红线**：动 storage 前读 `MASTER.md`；走 `pwa-cloud-store` skill。**版本：user 2026-09-19「可以 bump minor」→ 0.5.0 在纪元第一笔代码 commit 时花掉；prod 未推、未指示（E3「不做」照旧）。** 文件 = `.glb`（契约稿 rev3）；store 侧要一个 escalate = 通用「头片 peek」（封面，契约稿 §9）；无地 = WeebPaint 标准三态 + T-crash（A15 作废）。
...
- 设计稿 = `ai-docs/20260919-persistence-data-contract.md`（Claude Fable 5.1）。**rev3（2026-09-20）现状**：文件 = **一个 `.glb`**（glTF 2.0 二进制容器、目标语义 glTF 2.1：`asset.thumbnail` / `files`+`externalAssets`+`node.externalAsset` / `shapes`+`boundingVolume` / UID / 64 位 GLB v3；草案 schema 在 Khronos 仓 `draft-2.1` 分支 `specification/2.1/schema/`，规范 2026-06-11 公布、未定稿）；rev1 的 zip 薄壳作废（user「是否可以就一个 glb」）。core = bake、`CATSUP_document/brep/definitions/instance` = authoring、`extras` = ECS；丢进来的 glTF 模型 = 2.1 externalAssets 原子（§5.4）；B-rep 顶点整数微米（JSON 内联；大模型换 2.1 SIGNED_INT）；按 group 分池；无地 = WeebPaint 标准；演化 = 无兼容 + 后门；§8 = glTF 2.1 / 扩展注册表对照表（「你会内耗的清单」）。
- **grill 第二轮已答（user 2026-09-19/20）**：框架 = glTF 本身 ✔；一个 glb（待看 §2 点头）；整数「喜欢，谁要求的」→ §5.2 答（内核 1 µm 格 × float32 24 位尾数，三方不读）；`.glb`（跟容器一起）；无地按 WeebPaint 标准 → **A15 作废**；**可以 bump minor（0.5.0）**；embedding WeebPaint 进远景；GDTF 教训 =「被内容需求逼出的自适应格式」，**但 user 纠偏：CatsUp 是 SU 竞品 + data-driven 场景编辑器，核心本体定型（component / group / 丢 gltf 模型），GDTF 教训只管游戏层词汇** → rev3 已改。GDTF 原提案 = `~/jupyter/20260716 GDTF/20260814 meta/journals/old references/20260716 {PHILOSOPHY,Rough Design Idea}.md`，反省 = 同仓 `journals/20260814 复活initiative.md` + `20260814 feedback thread 1.md`。
- **第三轮已答（user 2026-09-20）**：(a) 一个 glb + `.glb` 同意；(b)「整数能存多大、要不要 32/64 两个选项」→ JSON 内联到 2⁵³（±9×10⁹ m），二进制按包围盒自动 int32/int64，不做用户选项；(c) store 头片 peek 同意（escalate 走 `pwa-cloud-store` skill，见 A18）；(d) 不懂 → 契约稿 §10 人话解释，默认沿用。**user「核心还是 sketchup，好好想一下」→ rev4 §5 = SketchUp .skp 本体逐项对照表**（Edge/Face/Curve/Group/Definition 行为 glue-to·cuts-opening·face-camera/Instance/文件当组件/Image/SectionPlane/Guide/Dimension/Text/Material 贴图尺寸/Tag/Scene 保存项/Style/Shadow+geo→烘太阳/ModelInfo/模型轴/Attributes），每项落哪、三方看到什么、哪个纪元实现。§3.1 答「和只支持 2.0 的工具兼容」= 兼容（容器 v2、minVersion 2.0、整数不进 core accessors）。**第四轮已答（user 2026-09-20）**：B-rep 全二进制 + bake 瘦身 + 体积预算（「要抠…N64 小模型包也要小」）；group = 哑变量（成员数组 + 轴框，无名 / 无态；「命名 = commitment and frozen，我怕命名」）；有机体 / 骨骼 / 高度图 / 体素 = 各自子结构扩展；**向后兼容 = 第一天立宪**（每子结构版本戳 + `src/format/migrate/` + 冻结样本；老文件永远能开）；**09-19 后门政策作废**（「我接受了 backward compatibility 之后就不需要后面啦」）；「第一天就做好完美 backward compatibility」「随着 gltf 进化对齐（体素）」「跨 gltf 版本兼容也要做」→ 契约 §7 第 9/10 款。**第五轮（09-20）**：马里奥 64 式面 tag = id + 名称表（extras 自定义）；最低位宽 = 最粗整除单位 + 包围盒相对无符号位宽 + meshopt；不存逐顶点 UV，面贴图定位 = 仿射/射影定点整数、autoUV 不存且算法版本化；顶点色 / 顶点光照 = 命名逐面角颜色层（rgb8，烘焙层 derived）。rev6。**第六轮（09-20）**：世界尺度契约 no farland（definition 4 000 km / 世界 9×10⁹ m、越界拒绝）；位宽阶梯含 uint8；平滑 = 逐边 smooth 位；几何零语义只认 tag；四档贴图定位（auto / 共享投影 / 面映射 / 显式面角 UV）+ 流自描述布局不分拍版本；§12 .blend/.skp/glTF 对照；**bake 移出契约**（两原则：省流、三方 WYSIWYG；保存选 wysiwyg / engine；细则归导出器 spec = A4 同一份）。rev7。**第七轮（09-20）**：「边 smooth 更好，normal/uv 已经那么多版本在打架」「巨构规范同意」「就是 do_versions，每子结构分拍更省迁移脚本与屎山」「应该可以开写了」→ **契约定稿，A18 开工**。**契约可开工；A18 等 user「开做」。**
...
### A15 无地期间的本地草稿持久化 — `作废（user 2026-09-19「无地按照 weebpaint 标准做」）`：无地 = WeebPaint 无地骑士宪法（transient / 文件家 / 图库家三态、IDB 永不当家、T-crash 盲快照；SSoT `../20260524 WeebPaint/ai-docs/20260825-localfile-knight-grill-verdicts.md` §1），映射见契约稿 §6。原提案留档供考古。（硬规则 #1 storage 红线，需 user 明批；user 2026-09-07：「idb 保留还是蛮重要的，即使是无地期间也鼓励我认真画东西，如何在数据契约还在大幅变动的现在实现这个但不屎山，也不是更新版本必丢？」）
...
- **B1 group 的精确形状**（group 纪元前）：AI 读法 = 同一顶点池 + context 标签（立宪页 A3 context 槽），group 变换烘进顶点坐标，渲染看不见 group；user 已澄清 drill L83「一个池」指物化层 VBO。要确认的推论：group 内 move 时边界是 sticky 判定的墙，但平面注册/面识别仍在同一世界坐标里跑。SSoT：`ai-docs/20260906-far-horizon-golden-format-and-ontology.md` §3.2。
  ...
  **2026-09-20 user 拍板（数据层）**：「我记得我拍过，group 是纯编辑逻辑。是一个数组。component 才是 object 层…我的 incentive 是 obj 需要命名。group 是你会频繁地合组解组用来编辑。命名 = commitment and frozen。我怕命名。group 是哑变量。就像用 tensor network 去逃 einsum 的上下表追踪」→ 文件里 group = 轴框 + 成员（每边/面一个组索引，同一池），**无名字 / 无 hidden / 无 locked / 无 tag / 无 attributes**（都在成员与 component 实例上）；契约稿 §5.1/§5.2 已落。运行时 sticky 墙推论仍待内核纪元核实。
...
- **B8** → 已转 A7（user 同意）。
- **B17 油漆桶（贴图纪元）注意事项**（user 2026-09-20：「转引擎的时候可以默认背材质是 culling。这样到时候油漆桶工具要小心（backlog 而不是 format spec 记这些 notes）」）`backlog`：① 面无朝向，两侧材质槽按平面 n 命名——油漆桶按射线打到的一侧涂（`dot(ray,n)` 符号），UI 要能看出涂的是哪一侧；② 引擎导出默认只保留有材质的一侧（背面 culling），两侧都涂才双面——涂反了在引擎里就是「面不见了」，需要提示 / 预览模式；③ 对象模式涂实例 = 重映射材质槽，编辑模式涂面 = 改 definition；④ 内核「换平面重拟合保 id」处反号时交换两槽（契约 §5.2「正反」）。
