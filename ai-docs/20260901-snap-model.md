...
**2026-09-06 补实「含点平面」（edited by Claude Fable 5.1）**：此前第二点查询只在过锚点的三个轴平面里挑、且先按俯视偏置取地面再吸附——从侧面底边往上拖矩形，光标射线穿过侧面落到地面底边附近，永远「边上」、矩形躺地（user 截图案）。现在：**光标射线命中的膜若也含锚点 → 该膜平面胜出**（`faceUnderCursor`，任意朝向的膜都算，非只轴向），否则退回锚轴/轴系。**线工具第二点同走此查询**（`Editor.lineSecondSnap`；此前锁死首点平面），落实 user 2026-09-01「线的空落点兜底=学矩形」。
