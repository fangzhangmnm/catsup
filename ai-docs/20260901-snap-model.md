...
> as-of v0.3.1 / 2026-09-06（创建 2026-09-01）· created by Claude Fable 5
...
**2026-09-06 补实「含点平面」（edited by Claude Fable 5.1）**：此前第二点查询只在过锚点的三个轴平面里挑、且先按俯视偏置取地面再吸附——从侧面底边往上拖矩形，光标射线穿过侧面落到地面底边附近，永远「边上」、矩形躺地（user 截图案）。现在：**光标射线命中的膜若也含锚点 → 该膜平面胜出**（`faceUnderCursor`，任意朝向的膜都算，非只轴向），否则退回锚轴/轴系。**线工具第二点同走此查询**（`Editor.lineSecondSnap`；此前锁死首点平面），落实 user 2026-09-01「线的空落点兜底=学矩形」。同日新增吸附种类 **`on-face`「面上」**（SU On Face）：`DrawPlane.face` 记平面出身（首点面锁 / 含点膜），解算只用到平面约束且点在该膜环内（含洞判定）→ 报「面上」、蓝球；悬停未落笔也显示（user：「还没有落笔的时候也应该显示面上的吸附」）。轴系/锚轴平面不带 face，永不报面上。
**同日事故 + 护栏（「一 snap 一 snap」）**：矩形/线第二点经 `resolveRectPlane` 时没传手中集，预演里自己的角点成了吸附目标，光标被上一帧的角拽住、出圈再跳（步长 = 端点 ε；lab 时代就有，ε 放大后显形）。护栏 = **`AlignQuery.hand` 改必填（表态制）**，空手显式写 `NO_HAND`；`resolvePlane`/`resolveRectPlane` 同。golden：`test/plane-second-point.test.ts`「矩形第二点不吸自己」。分层归属：对齐引擎（层 2）的查询契约 + 动词（层 3）构造查询时必须表态；C++ 里 = 无默认值的 struct 成员 + 同一条 golden。
事故记录：偏置曾抄三份，两次翻车（0.5 门槛头发丝、每帧重挑漏偏置）皆同源漂移——单一出处即药。

**2026-09-06 深夜补实「拖拽中的手中集」三律（edited by Claude Fable 5.1；user 案：「矩形侧面上往下拖一个矩形，很难吸附底边，或者干脆不吸附，有时候会吸附到这个面后面的某个底边」）**：
① **边级手中集 = 链式溶解**（`solver.edgeTargets`）：手中顶点若只是把旧边切开（切点两侧两边共线反向）→ 两侧并成一条载线段，旧边整条仍是目标；链两端任一端在手里才是手势边（追光标）→ 整链退赛。此前「任一端在手里整条退赛」：上一帧矩形一吸到底边、底边被切三段全退赛、下一帧又回来 → 横跳。
② **膜级手中集由工具表态**（`AlignHand.faces`）：draw 类（线/矩形）只报预演里 **BIRTH** 出来的膜（追光标的新膜）；DIVIDE 子膜与母膜同影柱、照常遮挡——此前触手膜全豁免，矩形一落在正面上正面就透明，背后底边露出来被吸。move 仍按顶点推（触手膜追光标）；pp 帽不透明不豁免。
③ ~~第二点平面黏性~~ → **近擦**（2026-09-07 改，`solver.nearFaceContaining` + `PLANE_NEAR_PX`=24px@800）：光标离含锚点膜的剪影 ≤ 24px 仍算在那张膜上（滑出底边不翻面），否则回老规则（面向度+落底偏置）。黏性版（沿用上一帧平面）出过仓库角点案：锚点=角点，光标离开时擦过竖墙 → 竖直 → 黏到空地成大竖板 / 拖到一半突然变竖直——历史态是错的一般化，撤；现规则纯几何无历史。
golden = `test/plane-second-point.test.ts`「侧面往下拖矩形吸底边」六案（含老定义露背对照组）。
④ **拾取同律（2026-09-07，user「high：有时候选择会选到面后面的东西」）**：`pick.pickEntity` 的顶点/边拾取同走 `occludedBy`（此前纯屏幕距离，膜后的角/边照样中选）；膜按最近 t 不变。golden `test/pick.test.ts`「膜后的顶点/边不可选」六案。
