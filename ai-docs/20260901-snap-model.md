...
## 拖拽中的吸附世界（2026-09-03 pp 抖动破案 v2，铁律）

**吸附世界不得是手势参数（h/delta）的函数。** 用 preview 当吸附世界时，目标的存亡随参数变：
推到底=湮灭→高度参考消失→掉回轴滑→h 回来→目标复活→再吸——`snap(world(h))→h′` 无不动点，
逐帧振荡（反馈回路探针 tools/probe-snap-jitter.mts 家族实锤）。

正解：**世界=旧核（静态不动点）− 手中集**（落笔时从旧核拓扑一次性取，整个手势不变）。
preview 的静态部分与旧核恒等（预演不新增静态几何），动态部分（手中几何+其旧位残影）由排除
谓词剔除——「不吸旧残影」语义完整保留。手中膜同时豁免遮挡（追 h 的膜遮挡目标=同一个回路）。
副作用=手中体对吸附透明（推拉时可隔着自己的墙吸到底部角点）：这是稳定性的代价，也是 SU 式
推拉 x-ray 手感；被吸住后持有稳定、光标离开 ε 一次性释放。
