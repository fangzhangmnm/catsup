...
## 7. snap 模型 grill 首轮裁决落地（同日，v0.0.13）

- **元逻辑三律**（S1-S4 标本收敛出）：①维度优先（点>线>面，低维赢家作废高维解读）
  ②延迟承诺（未决自由度留给后续点击）③单一供餐（工具不私自 raycast）。
- **Q1 已裁：充能制**（user「充能制度可以啊」）——from-point 源=anchor+原点（永久）+
  hover 端点/中点停留≥300ms 登记（LRU 3，紫点反馈）。**幽灵 align 结构病随全顶点常开一起死**
  （正交投影屏距无法定向深度，唯一不靠调参的解法=候选集缩到用户指过的点）。
  iPad 事实核查：**Safari 16.1 起 web 可拿 Apple Pencil 悬停**（16.4 补 tilt/azimuth；
  限 M2+ iPad + Pencil 2/Pro）→ 充能=渐进增强；通用兜底（最近吸附/落笔点自动充能）待下轮 grill。
- **铅笔终裁：回 SU 连画+出膜停**（含 DIVIDE 等任何膜事件=自动抬笔）；纯鼠标逃生=SU 官方
  口径 Esc（+原地点击）。此前「不连画」裁决被本裁决取代。
- **move 补裁**：有选区时作用于选区，**拾取点可点任何地方当参考点**（SU 精确移动语义）。
- 待 grill 续：ε 分层表、优先级全排序、词汇表 v1 范围、通用充能兜底、单一求解器落地时机。
